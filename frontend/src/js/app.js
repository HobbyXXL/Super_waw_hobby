...
const App = {
  setup() {
    ...
    const apiHobbies = ref([]);
    const feedLoading = ref(false);
    const newPostImageFile = ref(null);
    const toast = ref('');
    let toastTimer = null;
    ...
    function getNickFromEmail(em) {
      if (!em) return 'Гость';
      const nick = em.split('@')[0].replace(/[^a-zA-Zа-яА-Я0-9_]/g, '');
      return nick.length >= 3 ? nick : (nick + 'user').slice(0, 20);
    }
    function isValidEmail(em) { return /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(em.trim()); }

    async function loadApiHobbies() {
      try {
        apiHobbies.value = await hobbiesApi.list();
      } catch { apiHobbies.value = []; }
    }
    ...
    function applyProfileFromApi(profile) {
      ...
      if (profile.hobbies?.length) {
        currentUser.hobbies = profile.hobbies.map((h) => ({
          id: h.hobby_id,
          name: h.name || String(h.hobby_id),
    ...
    async function restoreSession() {
      ...
      try {
        const profile = await usersApi.meExtended();
    ...
    async function loadFeedFromApi(replace = false) {
      if (isGuest.value || !apiClient.getToken()) return;
      feedLoading.value = true;
      try {
        const items = await portfolioApi.feed({ skip: 0, limit: 20 });
        const mapped = (items || []).map((p) => mapFeedPost(p, currentUser.avatar));
        if (mapped.length) {
          if (replace) allPosts.value = [...mapped, ...allPosts.value.filter((p) => !p._fromApi)];
          else {
            const apiIds = new Set(mapped.map((p) => p.id));
            const rest = allPosts.value.filter((p) => !p._fromApi && !apiIds.has(p.id));
            allPosts.value = [...mapped, ...rest];
          }
        }
      } catch { /* keep mock feed */ }
    ...
    async function syncProfile() {
      ...
      try {
        ...
        if (stats?.current_streak != null) currentUser.streak = stats.current_streak;
      } catch { /* offline */ }
    }

    async function doCheckInForToday() {
      if (isGuest.value || !apiClient.getToken()) {
        markTodayVisited();
        currentUser.streak = updateStreak();
        return;
      }
      const rawId = selectedHobbies.value[0] || currentUser.hobbies[0]?.id;
      const hobbyId = findHobbyIdByUiId(rawId, apiHobbies.value, HOBBIES);
      if (!hobbyId) {
        markTodayVisited();
        return;
      }
      try {
        const res = await activitiesApi.checkIn(hobbyId);
        currentUser.streak = res.streak;
        markTodayVisited();
      } catch {
        markTodayVisited();
        currentUser.streak = updateStreak();
    ...
    async function submitAuth() {
      ...
      try {
        if (authMode.value === 'verify') {
          if (!verifyCode.value.trim() || verifyCode.value.trim().length !== 6) {
            authError.value = 'Введите 6-значный код из письма';
            return;
          }
          await authApi.verifyCode(pendingEmail.value, verifyCode.value.trim());
          const loginName = pendingLogin.value || getNickFromEmail(pendingEmail.value);
          const tokens = await authApi.login(loginName, pw);
          apiClient.setToken(tokens.access_token);
          apiClient.setStoredLogin(loginName);
          currentUser.name = loginName;
          isGuest.value = false;
          await loadApiHobbies();
          await syncProfile();
          playSoundIfEnabled('success');
          page.value = 'onboarding-hobbies';
          showToast('✅ Email подтверждён! Выберите хобби');
          return;
        }
        if (authMode.value === 'login') {
          const loginName = apiClient.getStoredLogin() || getNickFromEmail(em);
          const tokens = await authApi.login(loginName, pw);
          apiClient.setToken(tokens.access_token);
          apiClient.setStoredLogin(loginName);
          currentUser.name = loginName;
        ...
        const loginName = getNickFromEmail(em);
        await authApi.register(em, loginName, pw);
        pendingEmail.value = em;
        pendingLogin.value = loginName;
        apiClient.setStoredLogin(loginName);
        authMode.value = 'verify';
        showToast('📧 Код отправлен на почту (см. консоль сервера в DEBUG)');
      } catch (err) {
    ...
    function toggleHobby(id) {
      ...
      if (i === -1) {
        if (selectedHobbies.value.length >= 5) {
          onboardingValidationMsg.value = 'Можно выбрать не более 5 хобби';
          return;
        }
        selectedHobbies.value.push(id);
      } else {
        selectedHobbies.value.splice(i, 1);
      }
    ...
    function nextOnboarding() {
      ...
      if (page.value==='onboarding-hobbies') {
        ...
        currentUser.hobbies = selectedHobbies.value.map(id => { const h=HOBBIES.find(x=>x.id===id); return {id, name:h?.name||id, level:hobbyLevels.value[id]||'Новичок'}; });
      ...
      try {
        if (apiClient.getToken()) {
          const payload = buildProfilePayload(
            selectedHobbies.value, hobbyLevels.value, goalsText.value, apiHobbies.value, HOBBIES
          );
    ...
    async function submitPost() {
      ...
      if (newPostText.value.trim().length < 10) { postValidation.value = 'Текст поста — минимум 10 символов'; return; }
      authSubmitting.value = true;
      try {
        if (apiClient.getToken()) {
          const fd = new FormData();
          fd.append('title', newPostTitle.value.trim());
          fd.append('description', newPostText.value.trim());
        ...
        userStats.posts++;
        newPostTitle.value = ''; newPostText.value = ''; newPostTags.value = []; newPostTagInput.value = '';
        newPostHobbyDone.value = false; newPostImage.value = null; newPostImageFile.value = null;
        showCreatePost.value = false;
        playSoundIfEnabled('success');
        showToast('🎉 Пост опубликован!');
        nextTick(() => { const sc = getScroller(); if (sc) sc.scrollTo({ top: 0, behavior: 'smooth' }); });
      } catch (err) {
        postValidation.value = err.message || 'Не удалось опубликовать пост';
        playSoundIfEnabled('error');
      } finally {
        authSubmitting.value = false;
      }
    ...
    return {
      page, authMode, email, password, authError, emailError, pwError, verifyCode, authSubmitting, feedLoading, toast,
...
<div>
  ...
  <div v-if="page==='auth'" style="position:relative;min-height:100vh">
    ...
    <div class="auth-page">
      ...
      <div class="auth-card">
        <div class="auth-card-title">{{ authMode==='verify' ? '📧 Подтверждение email' : (authMode==='login' ? '👋 Добро пожаловать!' : '🌟 Создание аккаунта') }}</div>
        ...
        <div v-if="authMode==='verify'" class="form-group">
          <label class="form-label">Код из письма:</label>
          <div class="form-input-wrap">
            <span class="icon">🔢</span>
            <input class="form-input" v-model="verifyCode" @keyup.enter="submitAuth" maxlength="6" placeholder="123456"/>
          </div>
          <div style="font-size:12px;color:var(--gray-text);margin-top:5px">В DEBUG код печатается в консоли backend</div>
        </div>
        ...
        <div class="auth-footer">
          ...
          <div v-else-if="authMode==='login'" style="font-size:15px;color:var(--brown-dark);font-weight:500">
            Нет аккаунта? <span class="auth-link" @click="authMode='register'">Регистрация</span>
          </div>
          <div v-else-if="authMode==='verify'" style="font-size:15px;color:var(--brown-dark);font-weight:500">
            <span class="auth-link" @click="authMode='register'">← Изменить email</span>
          </div>
          <button class="btn-primary" :disabled="authSubmitting" @click="submitAuth">{{ authSubmitting ? '...' : (authMode==='verify' ? 'Подтвердить' : (authMode==='login' ? 'Войти' : 'Регистрация')) }}</button>
        </div>
      </div>
      <div style="margin-top:20px;text-align:center;color:rgba(92,61,30,.6);font-size:13px">API: localhost:8000 · логин = часть email до @</div>
    </div>
  </div>
  ...
  <div v-if="showCreatePost" class="modal-overlay" @click.self="showCreatePost=false">
    <div class="modal-box" style="max-width:580px">
      ...
      <div class="modal-footer">
        ...
        <button class="btn-primary" :disabled="authSubmitting" @click="submitPost">{{ authSubmitting ? '...' : (tr.btn_publish + ' 🚀') }}</button>
      </div>
    </div>
  </div>
