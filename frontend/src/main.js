...
const postsAPI = {
  ...
  create(post) { return request('/portfolio/', { method: 'POST', body: JSON.stringify(post) }); },
  like(postId) { return request(`/likes/${postId}`, { method: 'POST' }); },
  unlike(postId) { return request(`/likes/${postId}`, { method: 'DELETE' }); },
  addComment(postId, text) { return request(`/portfolio/${postId}/comments`, { method: 'POST', body: JSON.stringify({ text }) }); }
};

const onboardingAPI = {
  complete(payload) { return request('/users/me/profile', { method: 'PUT', body: JSON.stringify(payload) }); }
};
  ...
  setup() {
    ...
    async function loadFeed() {
      try {
        ...
        userStats.posts = allPosts.value.length;
    ...
    // --- Эти функции должны быть определены ДО return ---
    ...
    function addGoalHint(hint) {
      goalsText.value += (goalsText.value ? '\n' : '') + hint;
    }
